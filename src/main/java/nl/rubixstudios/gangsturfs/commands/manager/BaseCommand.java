...
public abstract class BaseCommand extends BukkitCommand {
    ...
    protected boolean checkDonorPermission(CommandSender sender, String permission) {
        if(!sender.hasPermission(permission)) {
            sender.sendMessage(Language.PREFIX + Color.translate("You need a <rank> rank, to perform this command.".replace("<rank>", ""))); //TODO Add to language
            sender.sendMessage(Language.PREFIX + Color.translate("Check out all our ranks on our store.")); //TODO Add to language
            return false;
        }
        return true;
    }

    protected boolean checkStaffPermission(CommandSender sender, String permission) {
        if(!sender.hasPermission(permission)) {
            sender.sendMessage(Language.PREFIX + Color.translate("You can't perform this command.")); //TODO Add to language
            return false;
        }
        return true;
    }
